...
DEFAULT_LANGUAGE = os.getenv('DEFAULT_LANGUAGE', 'es-CL')

# OpenAI voice names mapped to edge-tts equivalents
voice_mapping = {
    'alloy': 'es-CL-CatalinaNeural',
    'echo': 'es-CL-LorenzoNeural',
    'fable': 'es-MX-DaliaNeural',
    'onyx': 'es-MX-JorgeNeural',
    'nova': 'es-AR-ElenaNeural',
    'shimmer': 'es-AR-TomasNeural'
}
...
def generate_speech(text, voice, response_format, speed=1.3):
...
def speed_to_rate(speed: float) -> str:
    ...
    if speed < 0 or speed > 3:
        raise ValueError("Speed must be between 0 and 3 (inclusive).")
