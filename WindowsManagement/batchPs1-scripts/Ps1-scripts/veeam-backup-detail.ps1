...
foreach ($vmdata in $vmlist) {
    ...
    if ($BackupDetail -eq $true) {
        $Message = New-Object System.Net.Mail.MailMessage $EmailFrom, $EmailTo
        $VMSubject = "Backup Start: " + $vmdata[0]
        $Message.Subject = $VMSubject
        $Message.IsBodyHTML = $true
        $VMText = "Backup Has Started for " + $vmdata[0] + "<br>"
        $VMText = $VMText + "Quiescence: " + $vmdata[1] + "<br>"
        $VMText = $VMText + "Encryption: " + $vmdata[2]
        $Message.Body = $VMText 
    ...
    if ($failcheck -ne $null)  {
        ...
        $VMText = ($backup | Select-Object @{n="Name";e={($_.name).Substring(0, $_.name.LastIndexOf("("))}} ,@{n="Start Time";e={$_.CreationTime}},@{n="End Time";e={$_.EndTime}},Result,@{n="Details";e={$failcheck.Title}})
    } Else {
        ...
        $VMText = ($backup | Select-Object @{n="Name";e={($_.name).Substring(0, $_.name.LastIndexOf("("))}} ,@{n="Start Time";e={$_.CreationTime}},@{n="End Time";e={$_.EndTime}},Result,@{n="Details";e={($status | sort creationtime -Descending | select -first 1).Title}})
    }

    if ($BackupDetail -eq $true) {
        $Message = New-Object System.Net.Mail.MailMessage $EmailFrom, $EmailTo
        $VMSubject = "Backup End: " + $vmdata[0]
        $Message.Subject = $VMSubject
        $Message.IsBodyHTML = $true
        $Message.Body = $VMText | ConvertTo-Html -head $style | Out-String
        $SMTP = New-Object Net.Mail.SmtpClient($SMTPServer, $EMailPort)
        if ($EMailSSL -eq $true) {
            $SMTP.EnableSSL = $true
        }
