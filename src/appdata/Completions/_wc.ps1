...
Register-ArgumentCompleter -Native -CommandName 'wc' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'wc' {
            [CompletionResult]::new('--files0-from', '--files0-from', [CompletionResultType]::ParameterName, 'read input from the files specified by   NUL-terminated names in file F;   If F is - then read names from standard input')
            [CompletionResult]::new('--total', '--total', [CompletionResultType]::ParameterName, 'when to print a line with total counts;   WHEN can be: auto, always, only, never')
